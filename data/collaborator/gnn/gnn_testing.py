...
# -----------------------------
if len(sys.argv) < 2:
    raise ValueError("Missing collaborator id argument. Usage: python gnn_testing.py <collab_id>")

collab_id = sys.argv[1]
BASE_DIR = os.path.dirname(os.path.abspath(__file__))  # script dir — shared files live here
COLLAB_DIR = f"/home/nitin/software/data/collaborator/{collab_id}/gnn"
TEST_DIR = os.path.join(COLLAB_DIR, "testing_gnn", "year_month_files")
...
# -----------------------------
stations_file = os.path.join(COLLAB_DIR, "upstream_to_downstream_stations.csv")
...
model.load_state_dict(torch.load(os.path.join(BASE_DIR, "graphsage_model.pth")))
model.eval()
...
# SAVE CSV
# -----------------------------
out_csv = os.path.join(BASE_DIR, "wqi_predictions_detailed_test_graphsage.csv")
