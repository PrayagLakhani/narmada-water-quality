...
# -----------------------------
if len(sys.argv) < 2:
    raise ValueError("Missing collaborator id argument. Usage: python gnn_training.py <collab_id>")

collab_id = sys.argv[1]
BASE_DIR = os.path.dirname(os.path.abspath(__file__))  # script's own directory (shared files live here)
COLLAB_DIR = f"/home/nitin/software/data/collaborator/{collab_id}/gnn"
TRAIN_DIR = os.path.join(COLLAB_DIR, "training_gnn")
...
for epoch in range(1, epochs+1):
    ...
    if epoch >= min_save_epoch:
        ...
        if val_loss < best_val_loss:
            ...
            best_epoch = epoch
            torch.save(model.state_dict(), os.path.join(BASE_DIR, "graphsage_model.pth"))
            counter = 0
